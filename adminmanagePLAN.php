<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
       {
       $target_dir= "upload/";
       $target_file= $target_dir . basename($_FILES["floorplan"]["name"]);
       $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
      // $uploadOk = 1;

       $floorplan = basename($_FILES["floorplan"]["name"]); 
       $floorplan_name = isset($_POST['floorplan_name']) ? mysqli_real_escape_string($link,$_POST['floorplan_name']) : '';
       $building = isset($_POST['building']) ? mysqli_real_escape_string($link,$_POST['building']) : '';
       $floor = isset($_POST['floor']) ? mysqli_real_escape_string($link,$_POST['floor']) : '';
      
       $sql="INSERT INTO `rfid_2dplan` (`floorplan`, `floorplan_name`, `building`,`floor`) VALUES('$floorplan','$floorplan_name' ,'$building','$floor')";
       $result = mysqli_query($link,$sql);
        if (move_uploaded_file($_FILES["floorplan"]["tmp_name"], $target_file)) 
        {
            echo "The file ". $_FILES["floorplan"]["name"]. " has been uploaded.";
        } 
        else 
        {
        ...
        if($result==1)
        {
            phpAlert("การเพิ่มข้อมูลสำเร็จ");
        }
        else
        {
             phpAlert("การเพิ่มข้อมูลไม่สำเร็จ");
        }
       }
          ...
          <div class="row">
                    <div class="col-sm-12">
                        <div class="white-box" style="margin-left: 50px;margin-right: 50px">
                            <form name="submitfrm" method="post" enctype="multipart/form-data">
                               
                                    <!--Select image to upload--> 
                                    <input type="file" name="floorplan" id="floorplan" required=""/>
                                    <br>
                                    <br>
                                    <label for="Image">Input name</label>
                                    <input type="text" name="floorplan_name" id="floorplan_name" required=""/>
                                    <br>
                                    <br>
                                    <label for="Image">Input building</label>
                                    <input type="text" name="building" id="building" required=""/>
                                    <br>
                                    <br>
                                    <label for="Image">Input floor</label>
